...
class UBFC2Preprocess(BaseLoader):
    
    def __init__(self, data_path, des_path_root, face_size=128, frame_length=180, debug=False, detect_face_every_time=False):
        """Initializes the data loader.
        """    
        super(UBFC2Preprocess, self).__init__(data_path, des_path_root, face_size, frame_length, debug, detect_face_every_time)
    ...
    def extract_session(self, session, device):
        ...
        if len(wave_gt) < len(raw_frames) * 0.9 or len(raw_frames) < len(wave_gt) * 0.9:
    ...
    def read_wave(self, mata_data_file_path):
        ...
        with open(mata_data_file_path, "r") as f:
            ...
            wave_gt = gtdata[0] # 0 line is bvp
